		...
		>
			...
			<div>
				<div className="flex items-center gap-2">
					<p className={cx("font-medium text-sm", "text-neutral-900", "dark:text-neutral-100")}>{data.name}</p>
					{data.featured ? (
						<span
							className={cx(
				...
				<p className="text-neutral-500 text-sm dark:text-neutral-400">{data.description}</p>
			</div>
		</Link>
	)
}
	...
	<>
		...
		<div className="not-prose mt-8 grid gap-8">
			{Object.values(stack).map(({ items, type }) => (
				<Card key={type} title={type} className="grid gap-x-2 p-2 sm:grid-cols-2">
					{items.map(item => (
