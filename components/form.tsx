...
const Counter: FC<{ readonly count: number; readonly maxLength: number }> = ({ count, maxLength }) => (
	<p className="text-right text-neutral-500 text-xs dark:text-neutral-400">
		{count}/{maxLength}
	</p>
)

export const Input: FC<InputProperties> = ({ label, maxLength, onChange, ...properties }) => {
	const id = useId()
	...
	return (
		<fieldset className="space-y-1">
			<div className="flex items-center justify-between gap-4">
				...
				{maxLength ? <Counter count={count} maxLength={maxLength} /> : undefined}
			</div>
			<input
				id={id}
...
type TextareaProperties = CustomProperties & Omit<ComponentProps<"textarea">, "id">

export const Textarea: FC<TextareaProperties> = ({ label, maxLength, onChange, ...properties }) => {
	const id = useId()
	const [count, setCount] = useState(0)
	...
	return (
		<fieldset className="space-y-1">
			...
			<textarea
				id={id}
				placeholder="Jane Doe"
				className={cx(baseClassName, "max-h-[15rem] min-h-[5rem] resize-y")}
				maxLength={maxLength}
				onChange={handleChange}
				{...properties}
			/>
						...
						>
							{data.map(item => (
								...
								>
									...
									<span className="ml-2 hidden text-gray-500 sm:block dark:text-gray-400">{item.subtitle}</span>
								</RadixSelect.Item>
							))}
						</RadixSelect.Viewport>
